...
1. **NEVER use "(UNCHANGED IN VX)" markers in headers**
   - ❌ `#### 3. Order Status Table (UNCHANGED IN V2)`
...
❌ `**Column A: Order ID** - Unchanged from V1, see V1 docs`
...
1. **Infer which file(s) from user's context:**
   ...
   **Examples:**
   - "Let's add order status formulas to google sheets" → `googlesheet_formulas.md`
