...
// Test 6b: AGENTS.md is NOT in template (it's generated from CLAUDE.md at scaffold time)
test('AGENTS.md is not a template file (generated from CLAUDE.md by the CLI)', () => {
  const agentsPath = path.join(__dirname, 'template', 'AGENTS.md');
  assert(!fs.existsSync(agentsPath), 'AGENTS.md should not exist in template/ — it is generated by bin/appsheet-project.js');

  const cliPath = path.join(__dirname, 'bin', 'appsheet-project.js');
  const cliContent = fs.readFileSync(cliPath, 'utf8');
  assert(cliContent.includes('generateAgentsFile'), 'CLI should generate AGENTS.md from CLAUDE.md');
});
